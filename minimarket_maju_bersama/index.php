...
<head>
    ...
    <style>
        :root { --hijau-tua: #1A531A; --hijau-muda: #A4C639; --putih: #ffffff; --abu: #f4f7f6; }
        body { font-family: 'Segoe UI', sans-serif; background-color: var(--abu); margin: 0; display: flex; overflow-x: hidden; }
        
        ::-webkit-scrollbar { width: 8px; }
        ::-webkit-scrollbar-track { background: #f1f1f1; }
        ::-webkit-scrollbar-thumb { background: var(--hijau-muda); border-radius: 10px; }
        ::-webkit-scrollbar-thumb:hover { background: var(--hijau-tua); }

        .sidebar { width: 250px; background: linear-gradient(135deg, #1A531A 0%, #0a240a 100%); height: 100vh; color: white; position: fixed; padding-top: 20px; transition: width 0.3s cubic-bezier(0.4, 0, 0.2, 1); overflow: hidden; white-space: nowrap; z-index: 1000; box-shadow: 4px 0 15px rgba(0,0,0,0.1); }
        .sidebar h2 { text-align: center; font-size: 1.1rem; padding-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.1); transition: opacity 0.3s; }
        .sidebar a { display: flex; align-items: center; color: white; padding: 15px 25px; text-decoration: none; transition: 0.3s; font-size: 1rem; }
        .sidebar a:hover, .sidebar a.active { background: rgba(164, 198, 57, 0.2); color: var(--hijau-muda); border-left: 5px solid var(--hijau-muda); }
        .sidebar a .ikon { margin-right: 15px; font-size: 1.4rem; }
        
        .sidebar.collapsed { width: 70px; }
        .sidebar.collapsed h2 { opacity: 0; pointer-events: none; }
        .sidebar.collapsed .teks-menu { display: none; }
        .sidebar.collapsed a { padding: 15px 20px; justify-content: center; border-left: 5px solid transparent; }
        .sidebar.collapsed a:hover, .sidebar.collapsed a.active { border-left: 5px solid var(--hijau-muda); }
        .sidebar.collapsed a .ikon { margin-right: 0; font-size: 1.5rem; }

        .main-content { margin-left: 250px; width: calc(100% - 250px); padding: 30px; min-height: 100vh; transition: margin-left 0.3s ease, width 0.3s ease; }
        .main-content.expanded { margin-left: 70px; width: calc(100% - 70px); }
        
        .top-bar { background: var(--putih); padding: 15px 25px; border-radius: 8px; margin-bottom: 25px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
        .btn-toggle { background: none; border: none; font-size: 24px; cursor: pointer; color: var(--hijau-tua); margin-right: 15px; transition: 0.3s; }
        .btn-toggle:hover { color: var(--hijau-muda); }

        .section-card { background: var(--putih); padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border-top: 5px solid var(--hijau-tua); }
        h3 { color: var(--hijau-tua); margin-top: 0; border-bottom: 2px solid var(--hijau-muda); padding-bottom: 10px; margin-bottom: 20px; }

        .card-metrik { transition: all 0.3s ease; cursor: default; }
        .card-metrik:hover { transform: translateY(-8px); box-shadow: 0 12px 20px rgba(0,0,0,0.15); }

        table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        th { background: #f8f9fa; padding: 12px; text-align: left; color: var(--hijau-tua); border-bottom: 2px solid #dee2e6; }
        td { padding: 12px; border-bottom: 1px solid #eee; }
        
        .btn { padding: 10px 18px; border-radius: 6px; text-decoration: none; display: inline-block; border: none; cursor: pointer; font-size: 14px; transition: all 0.2s ease; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        ...
        .btn-hijau { background: var(--hijau-tua); color: white; }
        .btn-merah { background: #dc3545; color: white; }
        input[type="text"], input[type="password"], input[type="number"], input[type="date"], select { padding: 10px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 10px; font-family: inherit; outline: none; }
        input:focus, select:focus { border-color: var(--hijau-muda); box-shadow: 0 0 5px rgba(164, 198, 57, 0.5); }
    </style>
</head>
<body>
...
<div class="main-content" id="main-content">
    ...
    <?php elseif ($page == 'produk') : ?>
        <div class="section-card">
            ...
            <table>
                <tr>
                    <th>Kategori</th><th>Produk</th><th>Harga</th><th>Stok</th>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 15px;">
                ...
                <form method="GET" action="index.php" style="display: flex; gap: 10px; align-items: center;">
                    ...
                    <select name="filter" onchange="this.form.submit()" style="margin-bottom: 0; cursor: pointer;">
                        ...
                        if (!empty($_GET['tgl_spesifik'])) { $flt = 'custom'; }
                        ?>
            ...
            <table>
                ...
                <tbody>
                    ...
                    if($res_r && mysqli_num_rows($res_r) > 0) :
                        ...
                        <tr><td colspan='4' style='text-align:center; color:#888;'>Tidak ada transaksi pada periode ini.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
