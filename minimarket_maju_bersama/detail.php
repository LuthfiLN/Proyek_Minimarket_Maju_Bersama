...
<head>
    ...
    <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #f4f7f6; display: flex; justify-content: center; padding: 40px; margin: 0; }
        .struk-card { background: white; width: 100%; max-width: 450px; padding: 30px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); border-top: 6px solid #1A531A; }
        
        .header { text-align: center; border-bottom: 2px dashed #ccc; padding-bottom: 15px; margin-bottom: 15px; }
        .header h2 { margin: 0; color: #1A531A; font-size: 24px; }
        .header p { margin: 5px 0 0; color: #666; font-size: 14px; }
        
        .info { font-size: 14px; color: #444; margin-bottom: 20px; line-height: 1.6; }
        
        table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        th { text-align: left; border-bottom: 2px solid #eee; padding-bottom: 8px; color: #1A531A; font-size: 14px; }
        td { padding: 10px 0; border-bottom: 1px solid #f9f9f9; font-size: 14px; color: #333; }
        .td-right { text-align: right; }
        
        .total-box { border-top: 2px dashed #ccc; padding-top: 15px; display: flex; justify-content: space-between; align-items: center; }
        .total-teks { font-size: 16px; font-weight: bold; color: #333; }
        .total-harga { font-size: 20px; font-weight: bold; color: #FF8600; }
        
        .btn-group { display: flex; gap: 10px; margin-top: 30px; }
        .btn { flex: 1; padding: 12px; border: none; border-radius: 5px; cursor: pointer; text-align: center; text-decoration: none; font-size: 14px; font-weight: bold; }
        .btn-kembali { background: #666; color: white; }
        .btn-cetak { background: #1A531A; color: white; }
        .btn-baru { background: #FF8600; color: white; }
        
        @media print {
            .btn-group { display: none; }
            body { background: white; padding: 0; }
            .struk-card { box-shadow: none; border: none; }
        }
    </style>
</head>
<body>
